...
class RRuleHandler:
    ...
    def build_rrule(
        ...
        # Add interval
        if frequency_interval and frequency_interval > 1:
        ...
        if frequency_type == "weekly":
            if frequency_days:
                # frequency_days is comma-separated string like "0,2,4" (Mon, Wed, Fri)
                days_list = [int(d.strip()) for d in frequency_days.split(",")]
                # Map to two-letter weekday codes
                weekday_abbrev = {0: "MO", 1: "TU", 2: "WE", 3: "TH", 4: "FR", 5: "SA", 6: "SU"}
                weekday_strs = [weekday_abbrev[d] for d in days_list]
                rrule_parts.append(f"BYDAY={','.join(weekday_strs)}")
            elif day_of_week is not None:
                # Map to two-letter weekday code
                weekday_abbrev = {0: "MO", 1: "TU", 2: "WE", 3: "TH", 4: "FR", 5: "SA", 6: "SU"}
                weekday_str = weekday_abbrev[day_of_week]
